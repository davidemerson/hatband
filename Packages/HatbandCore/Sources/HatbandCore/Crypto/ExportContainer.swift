...
public enum ExportContainer {
    ...
    /// iteration count costs nothing. Anything the AEAD rejects, including a
    /// wrong passphrase, is `wrongPassphraseOrTampered`: the two are not
    /// distinguishable and should not be.
    public static func open(_ container: [UInt8], passphrase: String) throws -> [UInt8] {
        guard container.count <= maxContainerSize, let value = try? CBOR.decode(container),
              let map = value.mapValue, let version = value[0]?.unsignedValue
